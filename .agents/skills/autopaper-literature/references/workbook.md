...
固定编号 CSV 导入时，记录可另外携带 `sequence` 与 `folder_name`。工作簿保留原序号和目录；若已有 DOI 对应不同序号或目录，或新序号发生冲突，则拒绝写入。导入固定清单时新增“补充材料”页，逐附件记录 SI 状态、来源、本地路径、格式、字节数、SHA-256 和失败原因。只有 SI 的报告不更改“文献清单”中的正文状态。
